    ...
    *start_addr = wp;
    ...
    if (wp % lba_size == 0) {
        ...
        if ((size + curr_data_size_in_block) % lba_size != 0) {
            padding_size = ((size + curr_data_size_in_block) / lba_size) * (lba_size) + lba_size;
        } else {
            padding_size = size;
        }
...
int BlockManager::write(uint64_t lba, void *buffer, uint32_t size) {
    ...
    if (padding & cross_bd) {
        ...
        memcpy(new_blocks, before, curr_data_size_in_block);
        memcpy(new_blocks + curr_data_size_in_block, buffer, size);
        memcpy(new_blocks + curr_data_size_in_block + size, after + after_index, padding_size - size - curr_data_size_in_block);
