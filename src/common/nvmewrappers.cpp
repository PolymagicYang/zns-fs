...
int ss_nvme_write_zeros(int fd, __u32 nsid, __u64 slba, __u16 nlb, __u16 control,
		     __u32 reftag, __u16 apptag, __u16 appmask) {
    return nvme_write_zeros(fd, nsid, slba, nlb, control, reftag, apptag, appmask);
}
...
int ss_nvme_zns_mgmt_send(int t1, unsigned int t2, unsigned long long t3, bool t4, nvme_zns_send_action t5, unsigned int t6, void* t7) {
